...
// 정점포맷
//-------------------------------------------------------------
#define D3DFVF_CUSTOMVERTEX (D3DFVF_XYZRHW|D3DFVF_DIFFUSE|D3DFVF_TEX1)// ★★★변경
...
HRESULT CMyD3DApplication::InitDeviceObjects()
{
    ...
    if( FAILED( D3DXCreateTextureFromFile( m_pd3dDevice
					, "earth.bmp"			// 파일명
					, &m_pEarthTexture ) ) )// 텍스처 오브젝트
        return E_FAIL;

    // 눈 텍스처 읽기
    if( FAILED( D3DXCreateTextureFromFile( m_pd3dDevice
					, "cloud.bmp"			// 파일명
					, &m_pCloudTexture ) ) )// 텍스처 오브젝트
        return E_FAIL;
...
HRESULT CMyD3DApplication::RestoreDeviceObjects()
{
    ...
    // 합성방법 설정
	TSS( 0, D3DTSS_ALPHAOP,   D3DTOP_MODULATE);// 인수성분 곱셈
    TSS( 0, D3DTSS_ALPHAARG1, D3DTA_TEXTURE );
    TSS( 0, D3DTSS_ALPHAARG2, D3DTA_DIFFUSE );

#define ALPHA_TYPE 0	// 이 값을 바꾸면 합성방법이 바뀐다
...
HRESULT CMyD3DApplication::Render()
{
    ...
    if( SUCCEEDED( m_pd3dDevice->BeginScene() ) )
    {
		...
		RS(D3DRS_ALPHABLENDENABLE, FALSE);// 반투명으로 그리지 않음
		m_pd3dDevice->SetTexture( 0, m_pEarthTexture );
		m_pd3dDevice->DrawPrimitive( D3DPT_TRIANGLESTRIP, 0, 2 );
		
		RS(D3DRS_ALPHABLENDENABLE, TRUE);// 반투명으로 그림
...
{
    SAFE_RELEASE( m_pCloudTexture );	// 텍스처
    SAFE_RELEASE( m_pEarthTexture );	// 텍스처

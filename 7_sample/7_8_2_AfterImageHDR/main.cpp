...
HRESULT CMyD3DApplication::RestoreDeviceObjects()
{
	...
	if (FAILED(m_pd3dDevice->CreateTexture(
				MAP_WIDTH, MAP_HEIGHT, 1,
				D3DUSAGE_RENDERTARGET, D3DFMT_A8R8G8B8,
				D3DPOOL_DEFAULT, &m_pCurrentTex, NULL)))
		return E_FAIL;
	if (FAILED(m_pCurrentTex->GetSurfaceLevel(0, &m_pCurrentSurf)))
		return E_FAIL;
	//-----------------------------------------------------
	// 렌더링타겟 복구
	//-----------------------------------------------------
...
HRESULT CMyD3DApplication::Render()
{
    ...
    if( SUCCEEDED( m_pd3dDevice->BeginScene() ) )
    {
		...
		if(cnt < m_fTime){
			...
			// 렌더링타겟 변경
			//-------------------------------------------------
			m_pd3dDevice->SetRenderTarget(0, m_pCurrentSurf);
			...
			if( m_pEffect != NULL )
			{
				...
				D3DXMatrixRotationY( &mR,  1.0f*m_fTime );
			...
			if( m_pEffect != NULL ) 
			{
				...
				T2VERTEX Vertex[4] = {
					//   x         y         z     w     tu tv
					{     0.0f,       0.0f, 0.1f, 1.0f,   0+du, 0+dv,  0+du, 0+dv,},
					{MAP_WIDTH,       0.0f, 0.1f, 1.0f,   1+du, 0+dv,  1+du, 0+dv,},
					{MAP_WIDTH, MAP_HEIGHT, 0.1f, 1.0f,   1+du, 1+dv,  1+du, 1+dv,},
					{     0.0f, MAP_HEIGHT, 0.1f, 1.0f,   0+du, 1+dv,  0+du, 1+dv,},
				};
				m_pd3dDevice->SetFVF( D3DFVF_XYZRHW | D3DFVF_TEX2 );
				m_pEffect->SetTexture("SrcMap", m_pTex[1-m_id]);
				m_pEffect->SetTexture("CurrentMap", m_pCurrentTex);
				m_pd3dDevice->DrawPrimitiveUP( D3DPT_TRIANGLEFAN
									, 2, Vertex, sizeof( T2VERTEX ) );
...
{
	// 렌더링타겟
	SAFE_RELEASE(m_pCurrentSurf);
	SAFE_RELEASE(m_pCurrentTex);
